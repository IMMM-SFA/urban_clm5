...
contains
  ...
  subroutine UrbanFluxes (bounds, num_nourbanl, filter_nourbanl,                        &
    ...
    associate(                                                                & 
         ...
         t_whiteroof_scaling1    =>    temperature_inst%t_whiteroof_scaling1_lun  , & ! Output: [real(r8) (:)]  white roof scaling temperature (K)             
         t_whiteroof_scaling2    =>    temperature_inst%t_whiteroof_scaling2_lun  , & ! Output: [real(r8) (:)]  white roof scaling temperature (K)             
         t_whiteroof_scaling3    =>    temperature_inst%t_whiteroof_scaling3_lun  , & ! Output: [real(r8) (:)]  white roof scaling temperature (K)             
         t_roof_scaling4    =>    temperature_inst%t_roof_scaling4_lun  , & ! Output: [real(r8) (:)]  white roof scaling temperature (K)             
         t_whiteroof_scaling4    =>    temperature_inst%t_whiteroof_scaling4_lun  , & ! Output: [real(r8) (:)]  white roof scaling temperature (K)             
         t_roof_scaling5    =>    temperature_inst%t_roof_scaling5_lun  , & ! Output: [real(r8) (:)]  white roof scaling temperature (K)             
         t_whiteroof_scaling5    =>    temperature_inst%t_whiteroof_scaling5_lun  , & ! Output: [real(r8) (:)]  white roof scaling temperature (K)             
         ...
         ra_canyon_resistance           =>   frictionvel_inst%ra_canyon_resistance_patch               , & ! Output: [real(r8) (:)   ]  canyon resistance (s/m)    
         ...
         sabs_roof_surface           =>   energyflux_inst%sabs_roof_surface_lun             , & ! Output: [real(r8) (:)   ]  roof absorbed solar radiation (per unit ground area), roof (W/m**2)           
         sabs_whiteroof_surface      =>   energyflux_inst%sabs_whiteroof_surface_lun        , & ! Output: [real(r8) (:)   ]  white roof absorbed solar radiation (per unit ground area), roof (W/m**2)           
         sabs_greenroof_surface      =>   energyflux_inst%sabs_greenroof_surface_lun        , & ! Output: [real(r8) (:)   ]  green roof absorbed solar radiation (per unit ground area), roof (W/m**2)           
         lwup_roof_surface           =>   energyflux_inst%lwup_roof_surface_lun             , & ! Output: [real(r8) (:)   ]  roof upward longwave radiation (per unit ground area), roof (W/m**2)           
         lwup_whiteroof_surface      =>   energyflux_inst%lwup_whiteroof_surface_lun        , & ! Output: [real(r8) (:)   ]  white roof upward longwave radiation (per unit ground area), roof (W/m**2)           
         lwup_greenroof_surface      =>   energyflux_inst%lwup_greenroof_surface_lun        , & ! Output: [real(r8) (:)   ]  green roof upward longwave radiation (per unit ground area), roof (W/m**2)           
         lwnet_roof_surface          =>   energyflux_inst%lwnet_roof_surface_lun            , & ! Output: [real(r8) (:)   ]  roof net longwave radiation (per unit ground area), roof (W/m**2)           
         lwnet_whiteroof_surface     =>   energyflux_inst%lwnet_whiteroof_surface_lun       , & ! Output: [real(r8) (:)   ]  white roof net longwave radiation (per unit ground area), roof (W/m**2)           
         lwnet_greenroof_surface     =>   energyflux_inst%lwnet_greenroof_surface_lun       , & ! Output: [real(r8) (:)   ]  green roof net longwave radiation (per unit ground area), roof (W/m**2)           
      ...
      do iter = 1,niters
         ...
         do fc = 1,num_urbanc
            ...
            if (ctype(c) == icol_roof) then
               ...
               wtuq(c) = fwet_roof*(wtlunit_roof(l)*(1._r8 - white_roof_fraction - green_roof_fraction)/canyon_resistance(l))
               wtuq_roof(l) = wtuq(c)
               ...
               if (snow_depth(c) > 0._r8) then
                  fwet_whiteroof = min(snow_depth(c)/0.05_r8, 1._r8)
               else
                  fwet_whiteroof = (max(0._r8, h2osoi_liq(c,1)+h2osoi_ice(c,1))/pondmx_urban)**0.666666666666_r8
                  fwet_whiteroof = min(fwet_whiteroof,1._r8)
               end if
               if (qaf(l) > qg(c)) then 
                  fwet_whiteroof = 1._r8
               end if
               
               
               ! scaled latent heat conductance
               wtuq(c) = fwet_whiteroof*(wtlunit_roof(l)*white_roof_fraction/canyon_resistance(l))
               wtuq_whiteroof(l) = wtuq(c)
               ! unscaled latent heat conductance
               wtuq_whiteroof_unscl(l) = fwet_whiteroof*(1._r8/canyon_resistance(l))
               ...
               fwet_greenroof = 1._r8
               ! scaled latent heat conductance
               wtuq(c) = fwet_greenroof*(wtlunit_roof(l)*green_roof_fraction/canyon_resistance(l))
               wtuq_greenroof(l) = wtuq(c)
               ! unscaled latent heat conductance
               wtuq_greenroof_unscl(l) = fwet_greenroof*(1._r8/canyon_resistance(l))
      ...
      do f = 1, num_urbanp
         ...
         ra_canyon_resistance(p) = canyon_resistance(l)  !pass value to global variable
         ...
         t_whiteroof_scaling1(l) = 4.0_r8*sb*em_roof(l)*taf(l)**3.0_r8 +  cpair * forc_rho(g)/canyon_resistance(l)
         t_whiteroof_scaling3(l) = sb*em_roof(l)*taf(l)**4.0_r8
         t_roof_scaling4(l) = eflx_sh_roof(l) + lwup_roof_surface(l) - t_whiteroof_scaling3(l)
         t_whiteroof_scaling4(l) = eflx_sh_whiteroof(l) + lwup_whiteroof_surface(l) - t_whiteroof_scaling3(l)
         t_roof_scaling5(l) = t_roof_scaling4(l) / t_whiteroof_scaling1(l)
         t_whiteroof_scaling5(l) = t_whiteroof_scaling4(l) / t_whiteroof_scaling1(l)
      end do
