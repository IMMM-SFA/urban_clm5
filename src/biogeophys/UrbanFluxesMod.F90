...
!    real(r8) :: fwet_greenroof                                       ! fraction of green roof surface that is wet (-)
    ...
    associate(                                                                & 
         ...
         lwdown_roof_surface         =>   energyflux_inst%lwdown_roof_surface_lun           , & ! Output: [real(r8) (:)   ]  roof downward longwave radiation (per unit ground area), roof (W/m**2)           
         lwdown_whiteroof_surface    =>   energyflux_inst%lwdown_whiteroof_surface_lun      , & ! Output: [real(r8) (:)   ]  white roof downward longwave radiation (per unit ground area), roof (W/m**2)           
      ...
      do iter = 1,niters
         ...
         do fc = 1,num_urbanc
            ...
            if (ctype(c) == icol_roof) then
               ...
               ! scaled latent heat conductance
               wtuq(c) = fwet_greenroof(l)*(wtlunit_roof(l)*green_roof_fraction/canyon_resistance(l))
               wtuq_greenroof(l) = wtuq(c)
               ! unscaled latent heat conductance
               wtuq_greenroof_unscl(l) = fwet_greenroof(l)*(1._r8/canyon_resistance(l))
